...
| Headless plant package, command/health/captured-age/safe-action/deadline-monitor contract, frame corpus, lifecycle, or channel-policy changes | `bun run check:plant-boundary`, `bun run check:plant-frames`, and `bun run validate:all` |
...
command/health, profile-bound captured-age/safe-action, and receipt-anchored
active deadline-monitor candidate tests, digest-bound JavaScript/Rust frame corpus,
