...
| Headless plant foundation | Lifecycle events and future typed channel values | Separate dependency-free package; generation-guarded fail-closed lifecycle; fixed queue policies and explicit loss accounting; process-lifetime sticky safety latch; inert adapter with no action operation; production subprocess/network/filesystem/device I/O and external source reachability rejected | No authenticated ingress, trusted health, watchdog, governor, physical safe action, FCU adapter, or deadline evidence exists; self-check is not release authority evidence |
