...
| NCP opt-in | Realm/session/model text, RPC replies, command frames | Off-by-default feature and unregistered commands; bounded inputs/timeouts; response kind/session/version checks; `CommandPlant` validation, monotonic sequence, TTL/horizon enforcement, and final HOLD | Explicit integration wiring and target Engram/ACL/version evidence; no always-on loop exists |
