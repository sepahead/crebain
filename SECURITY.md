...
| Native NCP action/control opt-in | Realm/session/model text, Zenoh configuration, RPC replies, command frames | Off-by-default feature and unregistered commands; secure mode requires `NCP_ZENOH_CONFIG`; bounded inputs/timeouts; explicit `ok` plus kind/session/version checks; per-session lifecycle/subscriber cleanup; sanitized `CommandPlant`, sequence, TTL/horizon, raw ESTOP, and bounded final-HOLD attempt | Audit actual TLS identities, ACLs, certificates, and topology; config loading is not policy evidence; no always-on Engram/action loop exists. The separately integrated evidence producer does not activate these commands |
| Headless plant foundation | In-memory command/health reports, captured-read age and safe-action-row proposals, exact-profile opaque situation candidates, finite frame-convention vectors, lifecycle events, and typed channel values | Separate dependency-free package; inactive contract-v1 candidate rejects version/profile/session/action/TTL/frame/unit/nonfinite/instantaneous-speed mismatches, binds the local frame into profile identity, retains frame on validated velocity, and separates producer from internally stamped plant-local time; digest-bound JavaScript/Rust corpus proves exact same-physical-frame ENU/NED and FLU/FRD velocity-axis mappings and rejects local/body routes without attitude; canonical health path seals private immutable fields and validates declared profile/vehicle/source/stream-epoch/generation/frame-instance identity, strict per-publisher sequence, profile-local frame, SI units, plant-local observation times, finite local vectors, battery range, and explicit unknown/unavailable state before atomic replacement; checked loads compute coherent ages from one instant; the separate profile-bound classifier rejects zero limits or exact-profile mismatch, owns that coherent observation, and compares receipt plus seven observation ages with nonzero exclusive limits so equality is outside; the separate safe-action candidate rejects zero situation codes, empty/oversized/duplicate tables, exact-profile mismatch, and unmapped codes, copies borrowed rows into a fixed 255-slot table without heap allocation, supplies no default, and keeps plant intents distinct from untrusted ingress actions; boundary mutation and compile-fail API checks lock the concrete non-mixable health pair, non-cloneable mutable publisher, captured-read ownership, strict comparator, private fixed dispatch table, exact policy/request binding, non-cloneable selection, and no raw runtime endpoint, boolean aggregate, reader conversion, implicit action conversion, or policy consumption; generation-guarded lifecycle, fixed queue/loss policies, sticky safety latch, passive expiry, inert adapter, and no production subprocess/network/filesystem/device I/O or external source reachability | Candidate profile/frame/age limits and safe-action rows remain unapproved; within-limit-at-read is not current freshness, health, safety, eligibility, or authorization, and an opaque situation code is not authoritative state; the draft ODD's inclusive `<=200 ms` position/velocity condition is not implemented by the exclusive classifier; the candidate profile identity does not content-bind the separately supplied safe-action table, so different tables can assert the same identity; there is no trigger/state classifier, overlap precedence, default intent, or proof that Hold/Land/RTL/disarm is available or safe; frame-instance identity and same-origin/datum/body-point coincidence remain caller preconditions; declared health identities are not authenticated or adapter-attested, duplicate channel construction with one epoch is not globally prevented, and real FCU sampling/multi-message coherence is unproved; no parser, authenticated ingress/collector, durable anti-rollback, approved freshness/health/safe-action policy, suspend qualification, active watchdog, immediately-before-write governor, physical safe-action transaction, FCU adapter/failsafe, or deadline-latency evidence exists; self-check and component mechanics are not release authority evidence |
