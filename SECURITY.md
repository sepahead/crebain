...
| Headless plant foundation | In-memory command/health reports, captured-read age proposals, finite frame-convention vectors, lifecycle events, and typed channel values | Separate dependency-free package; inactive contract-v1 candidate rejects version/profile/session/action/TTL/frame/unit/nonfinite/instantaneous-speed mismatches, binds the local frame into profile identity, retains frame on validated velocity, and separates producer from internally stamped plant-local time; digest-bound JavaScript/Rust corpus proves exact same-physical-frame ENU/NED and FLU/FRD velocity-axis mappings and rejects local/body routes without attitude; canonical health path seals private immutable fields and validates declared profile/vehicle/source/stream-epoch/generation/frame-instance identity, strict per-publisher sequence, profile-local frame, SI units, plant-local observation times, finite local vectors, battery range, and explicit unknown/unavailable state before atomic replacement; checked loads compute coherent ages from one instant; the separate profile-bound classifier rejects zero limits or exact-profile mismatch, owns that coherent observation, and compares receipt plus seven observation ages with nonzero exclusive limits so equality is outside; boundary mutation and compile-fail API checks lock the concrete non-mixable health pair, non-cloneable mutable publisher, captured-read ownership, strict comparator, and no raw runtime snapshot endpoint, boolean aggregate, or reader conversion; generation-guarded lifecycle, fixed queue/loss policies, sticky safety latch, passive expiry, inert adapter, and no production subprocess/network/filesystem/device I/O or external source reachability | Candidate profile/frame/age limits remain unapproved; within-limit-at-read is not current freshness, health, safety, eligibility, or authorization, and recent unknown/unavailable state remains non-nominal; the draft ODD's inclusive `<=200 ms` position/velocity condition is not implemented by the exclusive classifier; frame-instance identity and same-origin/datum/body-point coincidence remain caller preconditions; declared health identities are not authenticated or adapter-attested, duplicate channel construction with one epoch is not globally prevented, and real FCU sampling/multi-message coherence is unproved; no parser, authenticated ingress/collector, durable anti-rollback, approved freshness/health policy, suspend qualification, active watchdog, immediately-before-write governor, physical safe action, FCU adapter/failsafe, or deadline-latency evidence exists; self-check and component mechanics are not release authority evidence |
