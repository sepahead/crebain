...
feature-gated raw NCP evidence producer. It also has a supervised Engram host
...
| Engram supervised host | Query parameters and parent-window messages | Document-lifetime embedded latch, exact loopback or Tauri origin, bounded session nonce, fresh per-document context nonce, exact parent source, strict `engram.host.v1` envelope, 8 KiB limit, read-only context path, and disabled native, external telemetry, and artifact paths | Keep native, external telemetry, artifact exchange, NCP, and plant paths disabled. Do not add a command message. |
