...
| Headless plant foundation | Lifecycle events and future typed channel values | Separate dependency-free package; generation-guarded fail-closed lifecycle; fixed queue policies and explicit loss accounting; process-lifetime sticky safety latch; passive generation-bound monotonic expiry; inert adapter with no action operation; production subprocess/network/filesystem/device I/O and external source reachability rejected | No authenticated ingress, trusted health, active watchdog, apply-time governor, physical safe action, FCU adapter, suspend qualification, or deadline-latency evidence exists; self-check and passive expiry mechanics are not release authority evidence |
