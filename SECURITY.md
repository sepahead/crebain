...
| Vite-dev NCP harness | Manual `window.__ncpDrone` calls with wire-shaped commands | Development-only/no transport; per-entity buffers; strict wire-0.6 active vec3/horizon bounds; local elapsed-time integration; malformed-call HOLD; raw ESTOP latch and freshness-reset | Not a live NCP/action-plane test; absent from production builds; never treat browser injection as Engram/ACL evidence |
