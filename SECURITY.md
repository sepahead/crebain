...
| Engram restricted host | Query parameters and parent-window messages | Document-lifetime embedded latch, exact loopback or Tauri origin, bounded session nonce, fresh per-document context nonce, monotonic heartbeat, native-IPC accessibility probe, exact parent source, strict `engram.host.v1` envelope, 8 KiB limit, read-only context path, and disabled native, external telemetry, and artifact paths | The handshake correlates a document. It does not authenticate or attest a process or build. Keep native, external telemetry, artifact exchange, NCP, and plant paths disabled. Do not add a command message. Engram must block the remote iframe where Tauri cannot isolate it. |
