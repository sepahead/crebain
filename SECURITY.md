...
| Headless plant foundation | Lifecycle events and future typed channel/snapshot values | Separate dependency-free package; generation-guarded fail-closed lifecycle; fixed queue policies and explicit loss accounting; non-consuming retained whole-snapshot register atomically associates caller-supplied generation and exact sequence metadata; process-lifetime sticky safety latch; passive generation-bound monotonic expiry; inert adapter with no action operation; production subprocess/network/filesystem/device I/O and external source reachability rejected | No authenticated ingress, trusted vehicle-health schema/provenance/freshness, generation validation, active watchdog, apply-time governor, physical safe action, FCU adapter, suspend qualification, or deadline-latency evidence exists; self-check, snapshot storage, and passive expiry mechanics are not release authority evidence |
