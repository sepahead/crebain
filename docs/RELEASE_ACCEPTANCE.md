...
|------|---------------------|---------------------|
| Local cross-language gate | `bun run validate:all` passes: Phase 0 baseline plus fail-closed self-test; NCP manifest/lock/doc coherence; frontend typecheck/lint/format/tests; Rust fmt/default check/test/clippy; NCP-feature clippy/tests | Any error, inventory/config/pin drift, stale normative NCP version, test failure, or clippy warning |
