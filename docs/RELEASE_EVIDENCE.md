...
evidence is `6a80b9e462a893ec459ee894e6c8ba9fe038f1f8`; a later evidence-only
...
exact-profile-bound captured-read age classifier plus an opaque-situation
safe-action dispatch candidate. Its component evidence does not promote the
system beyond L0 or substitute for deployed TLS/ACL/receiver, authenticated
FCU collection, approved age/state or content-bound safe-action policy,
authoritative state classification, current/apply-time checks, SITL, HIL, or
field evidence.

| Field | Evidence |
|-------|----------|
| Candidate source commit | `6a80b9e462a893ec459ee894e6c8ba9fe038f1f8` |
| Branch/tag | `main`; no release tag |
| GitHub Actions run | [CI 29253962612](https://github.com/sepahead/crebain/actions/runs/29253962612) passed on attempt 1; version/Phase-0 coherence, Linux feature gates, the inert plant boundary, frontend validation/bundle, frontend coverage, and Linux/macOS default plus NCP Rust jobs all succeeded |
...
| Hosted CodeQL | [CodeQL 29253962677](https://github.com/sepahead/crebain/actions/runs/29253962677) passed for Rust and JavaScript/TypeScript on the exact source on attempt 1 |
| Additional hosted policy | [OpenSSF Scorecard 29253962708](https://github.com/sepahead/crebain/actions/runs/29253962708) passed on the exact source on attempt 1 |
| Local `bun run validate:all` | Passed on the exact source: 305 frontend tests; 89 plant unit/integration tests plus 13 compile-fail doctests; 320 default all-target Rust tests plus 1 ignored generator; 411 NCP-feature all-target Rust tests plus 1 ignored generator; strict default/NCP/plant Clippy and Rustfmt; 133 inventoried surfaces, 17 hazards, 153 production files, 31 pinned configs, 114 Phase-0 fail-closed fixtures, 20 frame mutations, 115 plant-boundary mutations (64 health/freshness plus 51 safe-action), and 29 production-boundary fixtures |
| Local MSRV check | Rust 1.88 plant `cargo check --all-targets`, 89 all-target tests, and 13 compile-fail doctests passed; Rust 1.89 application `cargo check --locked --all-targets` passed; development and CI remain pinned to 1.91.1 |
| Frontend test/coverage/bundle result | 45 files and 305 local tests passed; exact-source `bun run check:bundle` passed at 432.3/700.0 kB gzipped with 29 production-boundary fixtures; hosted frontend and coverage jobs passed inside CI 29253962612 |
...
| Inert plant foundation | Dependency boundary, exact crate-root/API inventory, sealed-health/freshness/safe-action static mutation checks, compile-fail endpoint/ownership/conversion checks, strict Clippy, property/stress/headless/retained-snapshot/expiry/contract/frame/health/freshness/safe-action tests, digest-bound cross-language frame corpus, and self-check | Exact source passed locally and in the hosted plant job: 89 unit/integration tests plus 13 compile-fail doctests, 115 plant-boundary mutations (64 health/freshness plus 51 safe-action), 32 frame cases, and 20 frame mutations. The safe-action evidence is only an inert exact-profile, opaque-code, no-default lookup over caller-proposed rows; it is not profile content binding, authoritative state classification, an approved ODD matrix, action conversion, or a physical response. This is partial CB-028/CB-030/CTL-005/HAZ-006/HAZ-007 component evidence only—CTL-007 remains planned and HAZ-007 remains open; no authenticated FCU state, real aggregation coherence, approved freshness/safety policy, current/apply-time checking, durable epoch ownership, active watchdog, apply-time governor, adapter, or live authority exists |
...
| Supply chain | cargo-deny and `bun audit` | Audit 29242246397 passed on the last dependency-manifest change `09dd5ec`; source `6a80b9e` has no audit-workflow manifest diff |
| Static analysis | CodeQL JavaScript/TypeScript and Rust | Exact-source CodeQL 29253962677 passed for both languages on attempt 1 |
...
| Plant health and effect boundary | Authenticated FCU collector/source; multi-message coherence with oldest-constituent time; frame-reset identity; durable epoch ownership; approved freshness policy plus authoritative situation classification and a content-bound safe-action matrix; immediately-before-write generation/health checks; watchdog, governor, adapter, and SITL/HIL traces | Pending external vehicle/profile/topology and later plant components; the current opaque situation-dispatch candidate does not satisfy these gates |
