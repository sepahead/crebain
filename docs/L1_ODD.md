...
| ODD-07 | Localization | FCU estimator healthy; local pose/velocity age ≤200 ms; consistent ENU↔NED/FLU↔FRD transforms; simulation epoch monotonic | Profile-neutral same-frame-instance velocity-axis corpus is component-tested; frame-instance identity and same-origin/datum/body-point proof, profile selection, attitude/yaw/quaternions, points/covariance, reset/staleness, and live FCU interpretation remain pending |
| ODD-08 | Required state | Armed/landed/mode, estimator health, battery, fence, link, position and velocity are one atomic plant snapshot | Closed immutable in-memory candidate and coherent age mechanics are component-tested; authenticated FCU collection, aggregation coherence, approved freshness policy, and apply-time consumption remain pending |
