...
| ODD-06 | Timing | Nominal command rate 20 Hz; command age ≤150 ms at apply; plant watchdog safe transition ≤250 ms | Contract v1 structurally bounds requested TTL and separates producer/local time; an unwired component detects receipt-anchored absolute deadlines with one worker/slot and strict same-stream replacement. A separate unwired observation loads one coherent health snapshot before minting one private monotonic reference instant for health ages and the command's requested-lifetime relation (equality outside), but the requested lifetime is not this clause's approved 150 ms apply policy, the observation is remintable rather than command-content-bound, and it is not a write-adjacent atomic transaction. The 150 ms apply-age policy, 250 ms safe transition, runtime coupling, WCET/jitter, suspend, and fault-to-effect evidence remain pending |
| ODD-07 | Localization | FCU estimator healthy; local pose/velocity age ≤200 ms; consistent ENU↔NED/FLU↔FRD transforms; simulation epoch monotonic | Profile-neutral same-frame-instance velocity-axis corpus and profile-bound age-comparison mechanics are component-tested. One unwired apply-check observation can load coherent health first and then compute all eight health-age relations and command age relative to one later reference instant, but its command carries no `VehicleIdentity` or `LocalFrameInstanceIdentity`, so profile/generation equality does not prove the same vehicle/frame instance or add HAZ-005 evidence. Its caller-proposed classifier uses exclusive limits (`age < limit`), so it does not implement or approve this clause's inclusive `≤200 ms` condition. Frame-instance identity and same-origin/datum/body-point proof, profile selection/approval, attitude/yaw/quaternions, points/covariance, reset/current/immediately-before-write staleness, and live FCU interpretation remain pending |
| ODD-08 | Required state | Armed/landed/mode, estimator health, battery, fence, link, position and velocity are one atomic plant snapshot | Closed immutable in-memory candidate and eight coherent ages from one read are component-tested; an unwired observation loads that snapshot before minting one shared age-reference instant and keeps those relations beside command age and neutral lifecycle state/generation. It has no direct boolean accessor or `From` conversion to `bool` and supplies no aggregate/authorizing health/safety verdict, although callers can compare facts. It is not command-content-bound and matching retained IDs/TTL must never pair it as a checked token. It may succeed with any `PlantState`, stale ages, or unknown/unavailable health. Authenticated FCU collection, aggregation coherence, approved age/state policy, an immediately-before-write check, and authoritative consumption remain pending |
...
this matrix. The apply-check observation candidate records lifecycle state and
