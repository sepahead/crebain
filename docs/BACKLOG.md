...
| 4 | Extend regression benchmarks (weekly benchmark budgets exist for detector paths) to sensor fusion, transport event routing, and position history | Better latency visibility |
...
- CI test-count summaries and benchmark artifacts: the CI workflow writes
  frontend/Rust test-count step summaries, and the scheduled benchmarks
  workflow uploads result artifacts.
