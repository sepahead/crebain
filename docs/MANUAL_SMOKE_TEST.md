...
| Galadriel off-by-default gates | Default release lacks `ncp`; a non-feature binary with `ENABLE=1` fails; an `ncp` build with switch absent/`0` opens no producer; any ambiguous switch fails | ✅ component startup-policy tests; inspect exact artifact/features manually |  |
...
| Engram restricted view | Start the reviewed loopback frame. Confirm a fresh context and heartbeat. Confirm native Engram IPC is inaccessible. Stop replies and confirm Engram relocks the frame. Confirm Performance and Sensor Fusion start collapsed while standalone defaults stay expanded. | 🟡 shared vector/unit coverage; packaged hostile-frame test required |  |
