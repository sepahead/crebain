...
const MEASUREMENT_SCOPE: &str = "callLatencyMs measures one DetectorRuntime::detect call; runtimeReportedLatencyMs is the latency returned by that runtime; sequentialDetectorThroughputFps is 1000 times iterations divided by the sum of callLatencyMs because those samples are milliseconds; evidenceLoopWallMs additionally includes timing, first-output cloning, detection JSON serialization/digesting, and sample recording; initializationMs includes detector construction and Detector::warmup; fixture decoding, Tauri IPC, spawn_blocking queueing, camera transport, renderer load, and UI work are excluded";
